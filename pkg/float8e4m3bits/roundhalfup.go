...
// way to the right.
// exponentBits must be passed with the float8e4m3 bias applied and the
// scaleFactor as well
...
func roundHalfTowardsPositiveInf(signBit, exponentBits,
    ...
    // actually larger than the midpoint)
    if (mantissaExtraPrecision == f32Float8E4M3HalfSubnormalLSB) &&
        lostPrecision {
        exponentMantissaComposite += 1
        addedOne = true
    ...
    // In the case that we're halfway, we add 1, only if the sign was
    // positive, otherwise we truncate
    if (mantissaExtraPrecision == f32Float8E4M3HalfSubnormalLSB) &&
    ...
    // Result is larger if the input was positive and we added 1, or if 
    // the input was negative and we truncated
